...
def SAM_NonUniform_b(actions_n, pre_actions_n, all_observations, all_pre_observations, pre_terminals, agent_index, gamma):
    ...
    if all_observations.shape[0] == 2:
        pre_values = SAM_NonUniform_potential(pre_actions_n, all_pre_observations[:,:,2:6], agent_index)
        pre_values[pre_terminals] = 0
        current_values = SAM_NonUniform_potential(actions_n, all_observations[:,:,2:6], agent_index)
    elif all_observations.shape[0] == 4:
        pre_values = SAM_NonUniform_potential(pre_actions_n, all_pre_observations[:,:,2:10], agent_index)
        pre_values[pre_terminals] = 0
        current_values = SAM_NonUniform_potential(actions_n, all_observations[:,:,2:10], agent_index)
...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def update(self, agents, t):
        ...
        for i in range(num_sample):
            ...
            n_good_agents = len(target_act_next_n)-1
            if self.agent_index == 0:
                SAM_NonUniform_rew = 0
            else:
                if look_back:
                    SAM_NonUniform_rew = SAM_NonUniform_b(np.array(act_n[1:(n_good_agents+1)]), np.array(pre_act_n[1:(n_good_agents+1)]), 
                        np.array(obs_n[1:(n_good_agents+1)]), np.array(pre_obs_n[1:(n_good_agents+1)]), pre_terminal_n[0], self.agent_index, self.args.gamma)
                    if n_good_agents == 4:
