...
from maddpg.trainer.maddpg_adv import MADDPGAgentTrainer
...
def SAM_uniform_f(all_observations, all_next_observations, gamma):
    ...
    """
    # Check the number of good agents (2 or 4 in our experiments), which equals to the number of landmarks
    n_good_agents = all_next_observations.shape[0]
    SAM_uniform_values = gamma*SAM_uniform(all_next_observations[:,:,2:(2+2*n_good_agents)]) - \
                        SAM_uniform(all_observations[:,:,2:(2+2*n_good_agents)])
...
def train(arglist):
    with U.single_threaded_session():
        ...
        Agent_SAM_uniform = True
        n_good_agents = len(obs_n)-1
        print('Starting iterations...', 'Using SAM_uniform: ', Agent_SAM_uniform)
        while True:
            # get action
            ...
            ################################################# Apply SAM_uniform
            if Agent_SAM_uniform:
                SAM_uniform_rew_agent = SAM_uniform_f(np.array(obs_n[1:(n_good_agents+1)])[:, np.newaxis,:], 
                            np.array(new_obs_n[1:(n_good_agents+1)])[:, np.newaxis,:], arglist.gamma)
                # scale and shift the value to make it close to the scale of environment rewards
                SAM_uniform_rew_agent = 500*(0.003 + SAM_uniform_rew_agent)
            ...
            for i, agent in enumerate(trainers):
                if i==0:
                    ...
                    # Add SAM_uniform to environment rewards for all good agents
                    if Agent_SAM_uniform:
                        agent.experience(obs_n[i], action_n[i], rew_n[i]+SAM_uniform_rew_agent, new_obs_n[i], done_n[i], terminal)
                    else:
                        agent.experience(obs_n[i], action_n[i], rew_n[i], new_obs_n[i], done_n[i], terminal)
            #################################################
            ...
            for i, rew in enumerate(rew_n):
                ...
                if Agent_SAM_uniform:
                    episode_SAM_uniform[-1] += SAM_uniform_rew_agent
                else:
